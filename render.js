...
const POI = { water: ['#3E7FAF', 'drop'], bakery: ['#B8720A', 'sq'], shop: ['#B8720A', 'tri'], bike: ['#2F8F46', 'dia'], pharmacy: ['#2F8F46', 'plus'], hospital: ['#D71920', 'H'], pass: ['#17191C', 'pass'], peak: ['#17191C', 'peak'] };
...
export function createRenderer(canvas) {
  ...
  function draw(S) {
    ...
    // POIs
    if (z >= 13) for (const p of query(M.poiIndex, box)) { if (p.k.startsWith('place') || (z < 15 && (p.k === 'shop' || p.k === 'bakery' || p.k === 'pharmacy'))) continue; const q = toPx(p.lat, p.lon); if (q[0] < -20 || q[0] > W + 20 || q[1] < -20 || q[1] > H + 20) continue; poiIcon(p, q, z); }
  ...
  function poiIcon(p, q, z) {
    ...
    switch (s[1]) {
      ...
      case 'H': ctx.font = '800 10px Archivo'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('H', q[0], q[1] + 0.5); ctx.textAlign = 'left'; break;
