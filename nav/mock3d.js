...
import { loadRoutes } from './data-mod.js?v=c8177a80';
import { loadStage, pointAt, bearingAt } from './track.js?v=c8177a80';
import { mercX, mercY } from './geo.js?v=c8177a80';
import * as dem from './dem.js?v=c8177a80';
import * as sat from './sat.js?v=c8177a80';
import { flagAt } from './render.js?v=c8177a80';
...
async function main() {
  ...
  function plate(txt, color, w, h, y, g) { const c = document.createElement('canvas'); const cx = c.getContext('2d'); const font = '700 44px "Sofia Sans Semi Condensed", "Arial Narrow", sans-serif'; cx.font = font; const tw = Math.ceil(cx.measureText(txt).width) + 40; c.width = tw; c.height = 64; cx.fillStyle = color; cx.fillRect(0, 0, tw, 64); cx.fillStyle = color === '#FFE500' ? '#000' : '#fff'; cx.font = font; cx.textBaseline = 'middle'; cx.fillText(txt, 20, 34); const t = new THREE.CanvasTexture(c); t.colorSpace = THREE.SRGBColorSpace; const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: t, depthTest: false })); s.scale.set(h * tw / 64, h, 1); s.position.y = y; s.center.set(0.5, 0); s.renderOrder = 9; g.add(s); }
  ...
  function iconCol(name, ele) { const g = new THREE.Group(); disc(g, 4); cyl(0.25, 0.3, 6, INK, 0, 3.6, 0, g); box(4.8, 1.7, 0.25, ROUGE, 0, 6.2, 0, g); plate(name.toUpperCase() + ' · ' + ele + ' M', '#E3202E', 0, 2.4, 7.4, g); return g; }
