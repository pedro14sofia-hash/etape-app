...
async function main() {
  ...
  function plate(txt, color, w, h, y, g) { const c = document.createElement('canvas'); const cx = c.getContext('2d'); const font = '700 44px "Antonio", "Arial Narrow", sans-serif'; cx.font = font; const tw = Math.ceil(cx.measureText(txt).width) + 40; c.width = tw; c.height = 64; cx.fillStyle = color; cx.fillRect(0, 0, tw, 64); cx.fillStyle = color === '#FFFF00' ? '#000' : '#fff'; cx.font = font; cx.textBaseline = 'middle'; cx.fillText(txt, 20, 34); const t = new THREE.CanvasTexture(c); t.colorSpace = THREE.SRGBColorSpace; const s = new THREE.Sprite(new THREE.SpriteMaterial({ map: t, depthTest: false })); s.scale.set(h * tw / 64, h, 1); s.position.y = y; s.center.set(0.5, 0); s.renderOrder = 9; g.add(s); }
