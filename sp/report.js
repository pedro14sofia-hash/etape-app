import { tzHM, tzAt } from './geo.js?v=a4f89d61';
// Étape Navegar · report.js
// Relatório do dia: números, subidas, paradas, ingestão vs plano, exportação.
import { elevationAt } from './track.js?v=a4f89d61';
import { toGpx } from './telemetry.js?v=a4f89d61';
...
export function render(r, all) {
  ...
  ${podium}
  <div class="grid"><div><b>${n1(r.km)}</b><span>km</span></div><div><b>${fmtT(r.elapsed)}</b><span>total</span></div><div><b>${fmtT(r.stopped)}</b><span>parado</span></div><div><b>${n1(r.vmax)}</b><span>máx km/h</span></div></div>
  ...
  ${r.climbs.length ? `<h4>Subidas</h4><table>${r.climbs.map(c => `<tr><td>${cat(c.cat)}${esc(c.name)}</td><td class="r">${n1(c.len / 1000)} km · ${n1(c.pct)} %</td><td class="r">${c.vam != null ? fmtT(c.time) + ' · ' + c.vam + ' m/h' : (isFinite(c.time) ? fmtT(c.time) : '–')}</td></tr>`).join('')}</table>` : ''}
