...
export function tick(f, p, movingSec, now, ctx) {
  ...
  const sinceDrink = (movingSec - f.lastDrink) / 60, sinceEat = (movingSec - f.lastEat) / 60, off = p.off || {};
  if (!off.water && sinceDrink >= p.drinkEveryMin + f.snoozed.drink && (!f._dAt || now - f._dAt > 120000)) { f._dAt = now; f._dRep = (f._dRep || 0) + 1; if (f._dRep > 2) { f.snoozed.drink += p.drinkEveryMin; f._dRep = 0; } else ev.push({ kind: 'drink', level: 2, text: 'Beber', sub: Math.round(sinceDrink) + ' min sem beber · ' + Math.round(p.sipMl) + ' ml', speak: 'Hora de beber.' }); }
  if (!off.carbs && sinceEat >= p.eatEveryMin + f.snoozed.eat && (!f._eAt || now - f._eAt > 180000)) { f._eAt = now; f._eRep = (f._eRep || 0) + 1; if (f._eRep > 2) { f.snoozed.eat += p.eatEveryMin; f._eRep = 0; } else ev.push({ kind: 'eat', level: 2, text: 'Comer ' + p.biteG + ' g', sub: Math.round(sinceEat) + ' min sem comer', speak: 'Hora de comer.' }); }
  // atrasado: ingestão 20 % abaixo do plano na hora corrida
  // extras por hora (sódio à parte, cafeína, métricas do Pedro): lembram no intervalo da dose
  for (const x of p.extras || []) {
    if (!x.everyMin) continue; const last = (f.lastExtra || {})[x.id] || 0, since = (movingSec - last) / 60; f._xAt = f._xAt || {};
    if (since >= x.everyMin && (!f._xAt[x.id] || now - f._xAt[x.id] > 180000)) { f._xAt[x.id] = now; ev.push({ kind: 'metric', metric: x.id, level: 2, text: x.name + ' · ' + x.dose + ' ' + x.unit, sub: Math.round(since) + ' min desde a última', speak: 'Hora de ' + x.name.toLowerCase() + '.' }); }
  }
  const hours = movingSec / 3600, planW = p.waterPerHour * hours, planC = p.carbsPerHour * hours;
  if (!f.events.length || off.water) return ev;                 // sem confirmação nenhuma, não cobra atraso nem garrafa
...
export function confirm(f, p, kind, movingSec, amount) {
  ...
  const x = (p.extras || []).find(e => e.id === kind); if (x) { f.extra = f.extra || {}; f.extra[kind] = (f.extra[kind] || 0) + (amount || x.dose); f.lastExtra = f.lastExtra || {}; f.lastExtra[kind] = movingSec; }
...
export function status(f, p, movingSec, remKm, speedKmh) {
  ...
  return { water: f.water, waterPlan: p.waterPerHour * hours, waterTotal: p.waterPerHour * Math.max(totalH, 1), carbs: f.carbs, carbsPlan: p.carbsPerHour * hours, carbsTotal: p.carbsPerHour * Math.max(totalH, 1), sodium: f.sodium, sodiumPlan: p.sodiumPerHour * hours, sodiumTotal: p.sodiumPerHour * Math.max(totalH, 1),
    ...
    off: p.off || {}, extras: (p.extras || []).map(x => ({ ...x, taken: (f.extra || {})[x.id] || 0, plan: x.perHour ? x.target * hours : x.target, total: x.perHour ? x.target * Math.max(totalH, 1) : x.target, nextMin: x.everyMin ? Math.max(0, Math.round(x.everyMin - (movingSec - ((f.lastExtra || {})[x.id] || 0)) / 60)) : null })) };
}
