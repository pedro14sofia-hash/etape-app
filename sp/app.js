...
import { mercX, mercY, haversine } from './geo.js?v=a4f89d61';
import { loadMap, loadRoutes, loadParadas, poisNear, loadContours } from './data-mod.js?v=a4f89d61';
import { createRenderer, drawFita } from './render.js?v=a4f89d61';
import * as track from './track.js?v=a4f89d61';
import * as gps from './gps.js?v=a4f89d61';
import * as guide from './guide.js?v=a4f89d61';
import * as voice from './voice.js?v=a4f89d61';
import * as ui from './ui.js?v=a4f89d61';
...
import * as session from './session.js?v=a4f89d61';
import * as telemetry from './telemetry.js?v=a4f89d61';
import * as fuel from './fuel.js?v=a4f89d61';
import * as report from './report.js?v=a4f89d61';
import * as sat from './sat.js?v=a4f89d61';
import * as dem from './dem.js?v=a4f89d61';
import * as shade from './shade.js?v=a4f89d61';
import * as compass from './compass.js?v=a4f89d61';
import * as weather from './weather.js?v=a4f89d61';
import * as sensors from './sensors.js?v=a4f89d61';
import * as free from './free.js?v=a4f89d61';   // navegação livre (build de São Paulo, window.FREE)
import * as outing from './outing.js?v=a4f89d61';   // tela 02 · antes de sair
import * as plan from './plan.js?v=a4f89d61';   // Diário: rotas A→B e etapa refeita fora da rota
import * as native from './native.js?v=a4f89d61';   // casca nativa (N3a): barômetro, brilho, térmico
import * as sidecar from './sidecar.js?v=a4f89d61';   // Cinema pacote 2: telemetria a 10 Hz ao lado de cada clipe
import * as music from './music.js?v=a4f89d61';   // pacote 5: YouTube Music comandado pela casca, regras de pausa
import * as cinema from './cinema.js?v=a4f89d61';
import * as auto from './auto.js?v=a4f89d61';   // Cinema v2: gatilhos automáticos
import * as passeio from './passeio.js?v=a4f89d61';   // diário do passeio (bateria, gatilhos, alertas) guardado com os sidecars
import * as etapas from './etapas.js?v=a4f89d61';   // U2 · a folha Escolher etapa, no lugar do <select> da fita
import * as cinemaUi from './cinema-ui.js?v=a4f89d61';   // Anna: tela do Cinema e faixa da música   // Cinema pacotes 3 e 4: estado, botões físicos, bússola de luz (a tela é da Anna)
import * as ajustes from './ajustes.js?v=a4f89d61';   // U1: os nove painéis, no lugar do menu plano
import * as entrada from './entrada.js?v=a4f89d61';   // U2 · Primeira vez e Conteúdo novo pelo Wi-Fi
import * as gaveta from './gaveta.js?v=a4f89d61';     // U7 · os quatro apps de fora a um gesto da borda de cima
import * as casca from './casca/index.js?v=a4f89d61'; // ADR-0006 · as Capacidades da casca, um adapter cada
...
export function init() {
  ...
  if (r3dq !== '0') import('./rider3d.js?v=a4f89d61').then(async m => {   // avatar 3D ligado por padrão (pedido do Pedro em 06/09, no tamanho do ícone 2D); ?r3d=0 desliga, ?r3d=1 procedural
  ...
  setTimeout(() => { import('./router.js?v=a4f89d61').then(async m => { if (await m.load('graph.json')) router = m; }).catch(() => { }); }, 4000);
  ...
  if (window.FREE) { document.body.classList.add('free'); S.free = true; free.init(S); S.freeStage = S.stage; S.prefs.cam = '2d'; S.prefs.sat = false; if ($('btnCam')) $('btnCam').hidden = true; $('stageName').textContent = 'Navegação livre'; $('stageSub').textContent = 'São Paulo · sem traçado'; $('stageBtn').disabled = true; $('stageBtn').setAttribute('aria-label', 'Sem etapas para escolher no Diário'); R.centerOn(S.stage.pts[0][0], S.stage.pts[0][1]); R.setView(null, null, 16, 0);
    ...
    import('./router.js?v=a4f89d61').then(async m => { if (await m.load('graph.json')) router = m; }).catch(() => { });
    const q0 = new URLSearchParams(location.search);
    import('./diario.js?v=a4f89d61').then(m => { diario = m; m.init(diarioCtx()); if (S.session.state === 'idle' && !q0.get('vias') && !q0.get('to')) m.open(); }).catch(e => console.error(e)); }
...
function setCam(c) {
  ...
  if (c === 'tp') {
    ...
    if (t3d) { show(); return; }
    import('./terrain3d.js?v=a4f89d61').then(m => {
...
function showPreview(key) {
  ...
  if (key === 'trip') {
    ...
    if (dem.available()) import('./diorama.js?v=a4f89d61').then(m => { diorama = m; return m.build($('pvDio'), st, paradas, key).then(okd => { if (!okd) { showView('map'); ctl.hidden = true; } else { hint.textContent = 'arraste para girar · toque duplo liga o giro'; if (key === S.stage.key && S.proj && S.proj.dist > 0) diorama.setProgress(S.proj.dist); } }); });
    else { showView('map'); ctl.hidden = true; }
    import('./render.js?v=a4f89d61').then(m => m.drawProfile($('pvProf'), st, 0, S.theme, { labels: true, paradas }));
  }
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (/[?&]casca=fake\b/.test(location.search) && !window.EtapeNative) {
    try { const f = await import('./casca/fake.js?v=a4f89d61'); await f.instalar(new URLSearchParams(location.search).get('roteiro')); }
